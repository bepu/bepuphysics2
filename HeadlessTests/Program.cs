...
var demo = new DegenerateTreeDemo();
demo.Initialize();
...
public class DegenerateTreeDemo
{
    public void Initialize()
    {
        var pool = new BufferPool();
        var simulation = Simulation.Create(pool, new DemoNarrowPhaseCallbacks(new SpringSettings(30, 1)),
            new DemoPoseIntegratorCallbacks(new Vector3(0, -10, 0)), new SolveDescription(8, 1));

        var tree = LoadTree("C:\\Users\\WDAGUtilityAccount\\source\\repos\\tree.bin", pool);
        for (var i = 0; i < 3000; i++) Insert(ref tree, pool);
        for (var i = 0; i < 100; i++) tree.RefitAndRefine(pool, i, 10000000);

        Console.WriteLine(AlexDebug.Print(tree));

        var tree2 = RebuildTree(tree, pool);
        Console.WriteLine();
        Console.WriteLine("-----------------------------------------");
        Console.WriteLine();
    ...
    private unsafe Tree RebuildTree(Tree tree, BufferPool pool)
    {
        var tree2 = new Tree(pool, tree.LeafCount)
        {
            NodeCount = tree.LeafCount - 1,
            LeafCount = tree.LeafCount
        };
        pool.Take(tree.LeafCount, out Buffer<NodeChild> subTree);
        FillSubtree(subTree, tree);

        tree2.BinnedBuild(subTree, pool);
        tree2.RefitAndRefine(pool, 0, 1000f);
        pool.Return(ref subTree);
        return tree2;
    }
    ...
    private void Insert(ref Tree tree, BufferPool pool)
    {
        tree.Add(new BoundingBox(new Vector3(-0.49999994f), new Vector3(0.49999994f)), pool);
        tree.Add(new BoundingBox(new Vector3(-0.3f, -0.3f, -0.020537015f), new Vector3(0.3f, 0.3f, 0.020537015f)),
            pool);
    }

    private Tree LoadTree(string path, BufferPool pool)
    {
