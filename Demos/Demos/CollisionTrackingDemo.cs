...
public class CollisionTrackingDemo : Demo
{
    ...
    public struct PairCollision
    {
        ...
        /// <summary>
        /// Stores whether the contact manifold held in this instance is convex or nonconvex.<para/>
        /// If this is true, then only <see cref="Convex"/> is populated with valid values and <see cref="Nonconvex"/> contains undefined values.<para/>
        /// If this is false, then only <see cref="Nonconvex"/> is populated with valid values and <see cref="Convex"/> contains undefined values.
        /// </summary>
        [FieldOffset(8)]
        ...
        /// <summary>
        /// Contacts generated by the narrow phase if <see cref="IsConvex"/> is true. Otherwise, contains undefined values.
        /// </summary>
        [FieldOffset(12)]
        public ConvexContactManifold Convex;
        /// <summary>
        /// Contacts generated by the narrow phase if <see cref="IsConvex"/> is false. Otherwise, contains undefined values.
        /// </summary>
        [FieldOffset(12)]
        public NonconvexContactManifold Nonconvex;
    }
    ...
    public class CollisionTracker
    {
        ...
        public void Flush()
        {
            ...
            for (int i = 0; i < workerCaches.Length; ++i)
            {
                ...
                for (int j = 0; j < cache.Count; ++j)
                {
                    ...
                    Tracked.Values[elementIndex].Collisions.Add(entry.Contacts, pool);
                }
        ...
        private void ReportContacts<TManifold>(int workerIndex, ContactSource self, ContactSource other, bool otherIsA, ref TManifold contacts) where TManifold : unmanaged, IContactManifold<TManifold>
        {
            if (Tracked.ContainsKey(self))
            {
                ...
                pairContacts.Contacts.Other = other;
                pairContacts.Contacts.OtherIsPairA = otherIsA;
                if (typeof(TManifold) == typeof(ConvexContactManifold)) //This is a JIT-time constant.
                {
                    pairContacts.Contacts.IsConvex = true;
                    pairContacts.Contacts.Convex = Unsafe.As<TManifold, ConvexContactManifold>(ref contacts);
                }
                else
                {
                    pairContacts.Contacts.IsConvex = false;
                    pairContacts.Contacts.Nonconvex = Unsafe.As<TManifold, NonconvexContactManifold>(ref contacts);
                }
            }
        }
    ...
    static bool IsTouching<TManifold>(ref TManifold contacts) where TManifold : unmanaged, IContactManifold<TManifold>
    {
    ...
    static bool IsTouching(ref PairCollision collision)
    {
        return collision.IsConvex ? IsTouching(ref collision.Convex) : IsTouching(ref collision.Nonconvex);
    }
    ...
    bool PreviousContainsFeatureId(ref PairCollision pair, int featureId)
    {
        if (pair.IsConvex)
        {
            for (int i = 0; i < pair.Convex.Count; ++i)
            {
                if (pair.Convex.GetFeatureId(i) == featureId)
                    return true;
            }
        }
        else
        {
            for (int i = 0; i < pair.Nonconvex.Count; ++i)
            {
                if (pair.Nonconvex.GetFeatureId(i) == featureId)
                    return true;
            }
        }
        return false;
        ...
        for (int trackedIndex = 0; trackedIndex < collisionTracker.Tracked.Count; ++trackedIndex)
        {
            ...
            for (int otherIndex = 0; otherIndex < others.Collisions.Count; ++otherIndex)
            {
                ...
                if (indexInPrevious >= 0)
                {
                    ...
                    if (other.IsConvex)
                    {
                        for (int i = 0; i < other.Convex.Count; ++i)
                        {
                            if (!PreviousContainsFeatureId(ref previous, other.Convex.GetFeatureId(i)))
                                AddParticle(other.Convex.GetOffset(ref other.Convex, i), other.Convex.Normal, other.OtherIsPairA ? other.Other.Collidable : self.Collidable);
                        }
                    }
                    else
                    {
                        for (int i = 0; i < other.Nonconvex.Count; ++i)
                        {
                            if (!PreviousContainsFeatureId(ref previous, other.Convex.GetFeatureId(i)))
                                AddParticle(other.Nonconvex.GetOffset(ref other.Nonconvex, i), other.Nonconvex.GetNormal(ref other.Nonconvex, i), other.OtherIsPairA ? other.Other.Collidable : self.Collidable);
                        }
                    }
                }
                else
