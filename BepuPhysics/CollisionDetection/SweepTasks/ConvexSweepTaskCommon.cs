...
namespace BepuPhysics.CollisionDetection.SweepTasks
{
            ...
            where TPairDistanceTester : struct, IPairDistanceTester<TShapeWideA, TShapeWideB>
    {
            ...
            out float t0, out float t1, out Vector3 hitLocation, out Vector3 hitNormal)
        {
            return ConvexSweepTaskCommon.Sweep<TShapeA, TShapeWideA, TShapeB, TShapeWideB, TPairDistanceTester>(
                shapeDataA, shapeTypeA, localOffsetA, orientationA, velocityA,
                shapeDataB, shapeTypeB, localOffsetB, offsetB, orientationB, velocityB,
    ...
    class ConvexSweepTaskCommon
    {
        ...
        public static unsafe bool Sweep<TShapeA, TShapeWideA, TShapeB, TShapeWideB, TPairDistanceTester>(
            void* shapeDataA, int shapeTypeA, in Vector3 localOffsetA, in Quaternion orientationA, in BodyVelocity velocityA,
            void* shapeDataB, int shapeTypeB, in Vector3 localOffsetB, in Vector3 offsetB, in Quaternion orientationB, in BodyVelocity velocityB,
            float maximumT, float minimumProgression, float convergenceThreshold, int maximumIterationCount,
            out float t0, out float t1, out Vector3 hitLocation, out Vector3 hitNormal)
            where TShapeA : struct, IConvexShape
            where TShapeB : struct, IConvexShape
            ...
            where TPairDistanceTester : struct, IPairDistanceTester<TShapeWideA, TShapeWideB>
        {
            Debug.Assert(
                ...
                (shapeTypeA == default(TShapeB).TypeId && shapeTypeB == default(TShapeA).TypeId),
                "Sweep type requirements not met.");
            OffsetSweep sweepModifier;
            sweepModifier.LocalOffsetA = localOffsetA;
            sweepModifier.LocalOffsetB = localOffsetB;
            sweepModifier.RALength = localOffsetA.Length();
            sweepModifier.RBLength = localOffsetB.Length();
            if (shapeTypeA == default(TShapeA).TypeId)
            {
        ...
        {
            float GetBoundingSphereRadiusExpansion(float maximumT, float angularSpeedA, float angularSpeedB);
            ...
            void GetNonlinearVelocityContribution(out Vector<float> nonlinearVelocityContribution);
            void AdjustHitLocation(in BodyVelocity velocityA, ref Vector3 hitLocation, float t0);
        }

        struct UnoffsetSweep : ISweepModifier
        {
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            public void AdjustHitLocation(in BodyVelocity velocityA, ref Vector3 hitLocation, float t0)
            {
                hitLocation = hitLocation + t0 * velocityA.Linear;
            }
            ...
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            public float GetBoundingSphereRadiusExpansion(float maximumT, float angularSpeedA, float angularSpeedB)
            {
                return 0;
            }
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            public void GetNonlinearVelocityContribution(out Vector<float> nonlinearVelocityContribution)
            {
                nonlinearVelocityContribution = new Vector<float>();
            }
        }
        struct OffsetSweep : ISweepModifier
        {
            ...
            public float RALength, RBLength;
            public Vector<float> NonlinearContribution;
                ...
                ref Vector<float> samples, ref Vector3Wide sampleOffsetB, ref QuaternionWide sampleOrientationA, ref QuaternionWide sampleOrientationB)
            {
                ...
                //Integrate orientations to sample locations and use it to rotate the offsets into world space.
                var halfSamples = samples * 0.5f;
                Integrate(ref initialOrientationA, ref angularA, ref halfSamples, out sampleOrientationA);
                Vector3Wide.Broadcast(LocalOffsetA, out var localOffsetA);
                QuaternionWide.TransformWithoutOverlap(ref localOffsetA, ref sampleOrientationA, out var offsetA);
                Integrate(ref initialOrientationB, ref angularB, ref halfSamples, out sampleOrientationB);
                Vector3Wide.Broadcast(LocalOffsetB, out var localOffsetB);
                QuaternionWide.TransformWithoutOverlap(ref localOffsetB, ref sampleOrientationB, out var offsetB);
                Vector3Wide.Subtract(ref offsetB, ref offsetA, out var netOffsetB);
            ...
            public float GetBoundingSphereRadiusExpansion(float maximumT, float angularSpeedA, float angularSpeedB)
            {
                NonlinearContribution = new Vector<float>(RALength * angularSpeedA + RBLength * angularSpeedB);
                return
                    RALength * Math.Min(MathHelper.Pi, maximumT * angularSpeedA) +
                    RBLength * Math.Min(MathHelper.Pi, maximumT * angularSpeedB);
            }

            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            public void GetNonlinearVelocityContribution(out Vector<float> nonlinearVelocityContribution)
            {
                nonlinearVelocityContribution = NonlinearContribution;
            }
        }
            ...
            where TSweepModifier : ISweepModifier
        {
            ...
            shapeA.ComputeAngularExpansionData(out var maximumRadiusA, out var maximumAngularExpansionA);
            shapeB.ComputeAngularExpansionData(out var maximumRadiusB, out var maximumAngularExpansionB);
            var wALength = velocityA.Angular.Length();
            var wBLength = velocityB.Angular.Length();
            var angularNonlinearContribution = sweepModifier.GetBoundingSphereRadiusExpansion(maximumT, wALength, wBLength);
            if (!GetSphereCastInterval(offsetB, linearVelocityB, maximumRadiusA + maximumRadiusB + angularNonlinearContribution, out t0, out t1) || t0 > maximumT || t1 < 0)
            {
                //The bounding spheres do not intersect, or the intersection interval is outside of the requested search interval.
                hitLocation = default;
                hitNormal = default;
            ...
            var tangentSpeedA = new Vector<float>(maximumRadiusA * wALength);
            var tangentSpeedB = new Vector<float>(maximumRadiusB * wBLength);
            var maxAngularExpansionA = new Vector<float>(maximumAngularExpansionA);
            var maxAngularExpansionB = new Vector<float>(maximumAngularExpansionB);
            ...
            while (true)
            {
                ...
                Vector3Wide.Dot(ref normals, ref wideLinearVelocityB, out var linearVelocityAlongNormal);
                sweepModifier.GetNonlinearVelocityContribution(out var nonlinearVelocityContribution);
                linearVelocityAlongNormal += nonlinearVelocityContribution;
                //Note that, for any given timespan, the maximum displacement of any point on a body is bounded.
                //timeToNext = distances / 
                //  (linearContribution + 
                //   min(tangentSpeedA, maximumAngularExpansionA / timeToNext) + 
            ...
            //Console.WriteLine($"iteration count: {iterationIndex}");
            //If there was an intersection, we need to correct the hit location for the sample location.
            sweepModifier.AdjustHitLocation(velocityA, ref hitLocation, t0);
            return intersectionEncountered;
        }
