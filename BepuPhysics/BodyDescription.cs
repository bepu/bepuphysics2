...
namespace BepuPhysics
{
    ...
    public struct BodyVelocities
    {
        ...
        public static unsafe void GatherVelocities(ref Buffer<BodyVelocity> velocities, ref TwoBodyReferences references, int count, out BodyVelocities velocitiesA, out BodyVelocities velocitiesB)
        {
            ...
            for (int i = 0; i < count; ++i)
            {
                GatherVelocities(ref velocities, ref velocitiesA, ref baseIndexA, i);
                GatherVelocities(ref velocities, ref velocitiesB, ref baseIndexB, i);
            }
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static unsafe void ScatterVelocities(ref BodyVelocities source, ref Buffer<BodyVelocity> targets, ref int baseIndex, int innerIndex)
        {
            ...
            ref var sourceSlot = ref GatherScatter.GetOffsetInstance(ref source, innerIndex);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static unsafe void ScatterVelocities(ref BodyVelocities sourceVelocitiesA, ref BodyVelocities sourceVelocitiesB, ref Buffer<BodyVelocity> targetVelocities, ref TwoBodyReferences references, int count)
        {
            //Grab the base references for the body indices. Note that we make use of the references memory layout again.
            ref var baseIndexA = ref Unsafe.As<Vector<int>, int>(ref references.IndexA);
            ref var baseIndexB = ref Unsafe.As<Vector<int>, int>(ref references.IndexB);
