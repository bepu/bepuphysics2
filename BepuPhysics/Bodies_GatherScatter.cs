...
namespace BepuPhysics
{
    public partial class Bodies
    {
            ...
            where TAccessFilter : unmanaged, IBodyAccessFilter
        {
            ...
            if (Vector256.IsHardwareAccelerated && Vector<float>.Count == 8)
            {
...

                {
                    //Load every body for the first half of the motion state.
                    //Note that buffers are allocated on cache line boundaries, so we can use aligned loads for all that matters.
                    var m0 = empty0 ? Vector256<float>.Zero : Vector256.LoadAligned(s0);
                    var m1 = empty1 ? Vector256<float>.Zero : Vector256.LoadAligned(s1);
                    var m2 = empty2 ? Vector256<float>.Zero : Vector256.LoadAligned(s2);
                    var m3 = empty3 ? Vector256<float>.Zero : Vector256.LoadAligned(s3);
                    var m4 = empty4 ? Vector256<float>.Zero : Vector256.LoadAligned(s4);
                    var m5 = empty5 ? Vector256<float>.Zero : Vector256.LoadAligned(s5);
                    var m6 = empty6 ? Vector256<float>.Zero : Vector256.LoadAligned(s6);
                    var m7 = empty7 ? Vector256<float>.Zero : Vector256.LoadAligned(s7);

                    //m0 = Vector256.Create(00f, 01f, 02f, 03f, 04f, 05f, 06f, 07f);
                    //m1 = Vector256.Create(10f, 11f, 12f, 13f, 14f, 15f, 16f, 17f);
                    //m2 = Vector256.Create(20f, 21f, 22f, 23f, 24f, 25f, 26f, 27f);
                    //m3 = Vector256.Create(30f, 31f, 32f, 33f, 34f, 35f, 36f, 37f);
                    //m4 = Vector256.Create(40f, 41f, 42f, 43f, 44f, 45f, 46f, 47f);
                    //m5 = Vector256.Create(50f, 51f, 52f, 53f, 54f, 55f, 56f, 57f);
                    //m6 = Vector256.Create(60f, 61f, 62f, 63f, 64f, 65f, 66f, 67f);
                    //m7 = Vector256.Create(70f, 71f, 72f, 73f, 74f, 75f, 76f, 77f);

                    //Shuffle indices of one vector into 10325476, then blend with its neighbor to get interleaved 00224466 and 11335577.
                    var interleaveBlendMask = Vector256.Create(-1, 0, -1, 0, -1, 0, -1, 0).AsSingle();
                    var shuffledM0 = Vector256.Shuffle(m0, Vector256.Create(1, 0, 3, 2, 5, 4, 7, 6));
                    var evens01 = Vector256.ConditionalSelect(interleaveBlendMask, m1, shuffledM0);
                    var odds01 = Vector256.ConditionalSelect(interleaveBlendMask, shuffledM0, m1);
                    var shuffledM2 = Vector256.Shuffle(m2, Vector256.Create(1, 0, 3, 2, 5, 4, 7, 6));
                    var evens23 = Vector256.ConditionalSelect(interleaveBlendMask, m3, shuffledM2);
                    var odds23 = Vector256.ConditionalSelect(interleaveBlendMask, shuffledM2, m3);
                    var shuffledM4 = Vector256.Shuffle(m4, Vector256.Create(1, 0, 3, 2, 5, 4, 7, 6));
                    var evens45 = Vector256.ConditionalSelect(interleaveBlendMask, m5, shuffledM4);
                    var odds45 = Vector256.ConditionalSelect(interleaveBlendMask, shuffledM4, m5);
                    var shuffledM6 = Vector256.Shuffle(m6, Vector256.Create(1, 0, 3, 2, 5, 4, 7, 6));
                    var evens67 = Vector256.ConditionalSelect(interleaveBlendMask, m7, shuffledM6);
                    var odds67 = Vector256.ConditionalSelect(interleaveBlendMask, shuffledM6, m7);

                    //We have a couple of vectors of 00224466, we want 00004444 and 22226666. Likewise, 11335577 -> 11115555 and 33337777.
                    //Same idea as before: shuffle one of the two contributors and blend.
                    var interleaveBlendMask2 = Vector256.Create(0, 0, -1, -1, 0, 0, -1, -1).AsSingle();
                    var shuffledEvens01 = Vector256.Shuffle(evens01, Vector256.Create(2, 3, 0, 1, 6, 7, 4, 5));
                    var v22226666from1032 = Vector256.ConditionalSelect(interleaveBlendMask2, evens23, shuffledEvens01);
                    var v00004444from3210 = Vector256.ConditionalSelect(interleaveBlendMask2, shuffledEvens01, evens23);
                    var shuffledEvens45 = Vector256.Shuffle(evens45, Vector256.Create(2, 3, 0, 1, 6, 7, 4, 5));
                    var v22226666from5476 = Vector256.ConditionalSelect(interleaveBlendMask2, evens67, shuffledEvens45);
                    var v00004444from7654 = Vector256.ConditionalSelect(interleaveBlendMask2, shuffledEvens45, evens67);

                    var shuffledOdds01 = Vector256.Shuffle(odds01, Vector256.Create(2, 3, 0, 1, 6, 7, 4, 5));
                    var v33337777from1032 = Vector256.ConditionalSelect(interleaveBlendMask2, odds23, shuffledOdds01);
                    var v11115555from3210 = Vector256.ConditionalSelect(interleaveBlendMask2, shuffledOdds01, odds23);
                    var shuffledOdds45 = Vector256.Shuffle(odds45, Vector256.Create(2, 3, 0, 1, 6, 7, 4, 5));
                    var v33337777from4567 = Vector256.ConditionalSelect(interleaveBlendMask2, odds67, shuffledOdds45);
                    var v11115555from6745 = Vector256.ConditionalSelect(interleaveBlendMask2, shuffledOdds45, odds67);

                    if (filter.GatherOrientation)
                    {
                        var v0from32107654 = Vector256.WithUpper(v00004444from3210, Vector256.GetLower(v00004444from7654));
                        var v1from23016745 = Vector256.WithUpper(v11115555from3210, Vector256.GetLower(v11115555from6745));
                        var v2from10325476 = Vector256.WithUpper(v22226666from1032, Vector256.GetLower(v22226666from5476));
                        var v3from01234567 = Vector256.WithUpper(v33337777from1032, Vector256.GetLower(v33337777from4567));
                        orientation.X = Vector256.Shuffle(v0from32107654, Vector256.Create(3, 2, 1, 0, 7, 6, 5, 4)).AsVector();
                        orientation.Y = Vector256.Shuffle(v1from23016745, Vector256.Create(2, 3, 0, 1, 6, 7, 4, 5)).AsVector();
                        orientation.Z = Vector256.Shuffle(v2from10325476, Vector256.Create(1, 0, 3, 2, 5, 4, 7, 6)).AsVector();
                        orientation.W = v3from01234567.AsVector();
                    }
                    else
                    {
                        Unsafe.SkipInit(out orientation);
                    ...
                    {
                        var v4from32107654 = Vector256.WithLower(v00004444from7654, Vector256.GetUpper(v00004444from3210));
                        var v5from23016745 = Vector256.WithLower(v11115555from6745, Vector256.GetUpper(v11115555from3210));
                        var v6from10325476 = Vector256.WithLower(v22226666from5476, Vector256.GetUpper(v22226666from1032));
                        //var v7from10324567 = Vector256.WithLower(v33337777from4567, Vector256.GetUpper(v33337777from1032);
                        position.X = Vector256.Shuffle(v4from32107654, Vector256.Create(3, 2, 1, 0, 7, 6, 5, 4)).AsVector();
                        position.Y = Vector256.Shuffle(v5from23016745, Vector256.Create(2, 3, 0, 1, 6, 7, 4, 5)).AsVector();
                        position.Z = Vector256.Shuffle(v6from10325476, Vector256.Create(1, 0, 3, 2, 5, 4, 7, 6)).AsVector();
                    }
                    else
                    {
                        Unsafe.SkipInit(out position);
