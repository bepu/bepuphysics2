...
namespace BepuPhysics.Trees
{
    ...
    /// </summary>
    public struct RayBatcher2 : IDisposable
    {
        ...
        public RayBatcher2(BufferPool pool, int rayCapacity = 2048, int treeDepthForPreallocation = 24) : this()
        {
            this.pool = pool;
            batchRayCount = 0;
        ...
        unsafe void TestNode<TRaySource>(Node* node, byte depth, ref TRaySource raySource) where TRaySource : struct, ITreeRaySource
        {
            ...
            {
                var rayIndex = raySource[i];
                var ray = (TreeRay*)batchRays.Memory + rayIndex;
                var aIntersected = Tree.Intersects(ref node->A.Min, ref node->A.Max, ray, out var tA);
                var bIntersected = Tree.Intersects(ref node->B.Min, ref node->B.Max, ray, out var tB);
                ...
                ushort rayStackEntry = (ushort)rayIndex;
                if (aIntersected)
                {
                    if (bIntersected)
                    {
                        if (tA < tB)
                        {
                            rayIndicesA0[stackPointerA0++] = rayStackEntry;
                            rayIndicesB[stackPointerB++] = rayStackEntry;
                        }
                        else
                        {
                            rayIndicesB[stackPointerB++] = rayStackEntry;
                            rayIndicesA1[stackPointerA1++] = rayStackEntry;
                        }
                    }
                    else
                    {
