...
partial struct Tree
{
    ...
    public unsafe int Add4(BoundingBox bounds, BufferPool pool)
    {
        ...
        //Try rotating the root to compensate for common pathological insertion orders.
        {
            var rootIndex = 0;
            ...
            if (float.Min(leftRotationCostChange, rightRotationCostChange) < 0)
            {
                ...
                if (leftRotationCostChange < rightRotationCostChange)
                {
                    ...
                    Metanodes[nodeIndexToReplace] = new Metanode { Parent = rootIndex, IndexInParent = 0 };
                    if (childToShiftUp.Index < 0) Leaves[Encode(childToShiftUp.Index)] = new Leaf(rootIndex, 1); else Metanodes[childToShiftUp.Index] = new Metanode { Parent = rootIndex, IndexInParent = 1 };
                ...
                else
                {
                    ...
                    Metanodes[nodeIndexToReplace] = new Metanode { Parent = rootIndex, IndexInParent = 1 };
                    if (childToShiftUp.Index < 0) Leaves[Encode(childToShiftUp.Index)] = new Leaf(rootIndex, 0); else Metanodes[childToShiftUp.Index] = new Metanode { Parent = rootIndex, IndexInParent = 0 };
                    if (nodeToReplace.A.Index < 0) Leaves[Encode(nodeToReplace.A.Index)] = new Leaf(nodeIndexToReplace, 0); else Metanodes[nodeToReplace.A.Index] = new Metanode { Parent = nodeIndexToReplace, IndexInParent = 0 };
                    if (nodeToReplace.B.Index < 0) Leaves[Encode(nodeToReplace.B.Index)] = new Leaf(nodeIndexToReplace, 1); else Metanodes[nodeToReplace.B.Index] = new Metanode { Parent = nodeIndexToReplace, IndexInParent = 1 };
                }
                //Validate();
